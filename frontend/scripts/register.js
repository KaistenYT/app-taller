window.addEventListener('DOMContentLoaded', () => {
  ...
  function validateForm() {
    ...
    const ok = username.length > 0 && password.length >= 4 && password === confirm;
    btn.disabled = !ok;
  }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!username || password.length < 4) {
      showAlert('danger', 'Usuario y contraseña deben tener al menos 4 caracteres');
      return;
    }
    ...
    try {
      ...
      let userMessage = 'Ocurrió un error al registrar el usuario. Intenta nuevamente más tarde.';

      if (msg.includes('usuario ya existe') || msg.includes('username exists')) {
        userMessage = 'El nombre de usuario ya está registrado. Intenta con otro.';
      } else if (msg.includes('credenciales inválidas') || msg.includes('invalid credentials')) {
        userMessage = 'Las credenciales proporcionadas no son válidas.';
      } else if (msg.includes('api de electron no disponible')) {
        userMessage = 'No se pudo conectar con el sistema interno. Contacta al soporte técnico.';
      }
