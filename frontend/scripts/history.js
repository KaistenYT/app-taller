window.addEventListener('DOMContentLoaded', () => {
    ...
    try {
      ...
      if (!rows || rows.length === 0) {
        ...
        for (const r of rows) {
          ...
          // Format data
          const client = r.client_name ? `${r.client_name}<br><small class="text-muted">${r.client_id || ''}</small>` : r.client_id || '<span class="text-muted">N/A</span>';
          const device = r.device_description ? `${r.device_description}<br><small class="text-muted">${r.device_serial || ''}</small>` : r.device_serial || r.device_id || '<span class="text-muted">N/A</span>';
          ...
          const config = actionConfig[r.action] || { color: 'secondary', icon: 'circle' };
      ...
      showLoading(false);
      showAlert('danger', 'Error al cargar el historial. Por favor, intente nuevamente.');
    }
  }
  ...
  function downloadCSV(rows) {
    if (!rows || rows.length === 0) {
      return showAlert('warning', 'No hay datos para exportar. Intenta ajustar los filtros.');
    }
    ...
    showAlert('success', `Archivo CSV exportado exitosamente (${rows.length} registros)`);
  }
  ...
  // Real-time filter on reception ID
  if (fReceptionId) {
    fReceptionId.addEventListener('input', debounceSearch);
  }
  
  // Real-time filter on select changes
  if (fAction) {
